...
const SupervisorDashboard: React.FC = () => {
  ...
  // Estados para Impresos Inventarios (Imagen 4)
  ...
  // Estados para Impresos Tarifas (Imagen 5)
            ...
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm animate-fade-in">
                <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl overflow-hidden">
                    ...
                    <div className="p-8 border-t border-gray-100 flex flex-col items-center">
                        <button onClick={handleGenerateInventory} className="bg-brand-600 hover:bg-brand-700 text-white px-10 py-4 rounded-xl font-bold flex items-center gap-3 transition-all active:scale-95 shadow-lg shadow-brand-200">
                        ...
                        <section className="space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-6">
                                <div className="p-4 bg-gray-50 rounded-xl flex items-center justify-between border">
                                    ...
                                    <input type="color" value={tarHeaderColor} onChange={e => setTarHeaderColor(e.target.value)} className="w-8 h-8 cursor-pointer rounded" />
                                </div>
                                <div className="p-4 bg-gray-50 rounded-xl flex items-center justify-between border">
                                    <span className="text-xs font-bold">Texto Info</span>
                                    <input type="color" value={tarMarginColor} onChange={e => setTarMarginColor(e.target.value)} className="w-8 h-8 cursor-pointer rounded" />
                                </div>
                            </div>
                        </section>
                        <section className="space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-6">
                                <div className="space-y-1">
                                    <label className="text-xs font-bold">¿Mostrar PVP?</label>
                                    <select value={tarShowPvp} onChange={e => setTarShowPvp(e.target.value)} className="w-full p-3 border rounded-xl bg-gray-50 font-bold text-sm">
                                        <option value="Si">Si</option>
                                        <option value="No">No</option>
                                    </select>
                                </div>
                                <div className="space-y-1">
                                    <label className="text-xs font-bold">Fecha Revisión</label>
                                    <input type="date" value={tarRevisionDate} onChange={e => setTarRevisionDate(e.target.value)} className="w-full p-3 border rounded-xl bg-gray-50 font-bold text-sm" />
                                </div>
                            </div>
                        </section>
                    </div>
                    <div className="p-8 border-t border-gray-100 flex justify-center">
                        <button onClick={handleGenerateTariff} className="bg-brand-600 hover:bg-brand-700 text-white px-10 py-4 rounded-xl font-bold shadow-lg">
        ...
        <div className="h-screen bg-[#f3f4f6] dark:bg-slate-950 flex flex-col font-sans overflow-hidden">
            <header className="bg-white dark:bg-slate-900 h-16 px-6 flex justify-between items-center shadow-sm border-b dark:border-slate-800 z-20">
                ...
                <div className="flex items-center gap-6">
                    <ThemeToggle />
                    <button onClick={logout} className="text-slate-400 hover:text-red-500"><LogoutIcon className="w-6 h-6" /></button>
                </div>
            </header>
            <main className="flex-1 overflow-auto p-8 custom-scrollbar">
                <div className="max-w-7xl mx-auto">
                    <button onClick={() => setView('menu')} className="flex items-center gap-2 text-brand-600 font-bold text-xs mb-8 uppercase tracking-widest">
                        <ArrowLeftIcon className="w-4 h-4" /> Volver al Panel
                    </button>
                    {renderViewContent()}
                </div>
            </main>
